    ...
    <>
      ...
      >
        <nav className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6 md:px-10">
          ...
          <div data-desktop-nav className="hidden items-center gap-8 lg:flex">
            {navLinks.map((l) => (
              <button
                key={l.id}
                onClick={() => go(l.id)}
            ...
            <Magnetic>
              <a
                ...
                className="inline-flex min-h-11 items-center rounded-full border border-line px-4 py-2 text-sm text-ink transition-colors hover:border-accent hover:bg-accent/10"
              >
                Get in touch
              </a>
            </Magnetic>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-0 z-[60] flex flex-col justify-start gap-2 overflow-y-auto bg-base/95 px-8 pb-[max(2rem,env(safe-area-inset-bottom))] pt-[max(5.5rem,env(safe-area-inset-top))] backdrop-blur-xl lg:hidden"
          >
            <button
              type="button"
              onClick={() => {
